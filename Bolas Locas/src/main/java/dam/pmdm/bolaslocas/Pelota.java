...
public class Pelota {
    ...
    public Pelota(Juego juego, float radio, float x, float y, float velocidad, float direccion) {
        this(juego, radio, x, y, velocidad, direccion, Colores.getPredefinido());
    }
    ...
    public boolean dentro(float x, float y) {
        return (float) Math.sqrt(Math.pow(centro.x - x, 2) + Math.pow(centro.y - y, 2)) <= radio;
    }
